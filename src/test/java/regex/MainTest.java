...
public class MainTest {
    ...
    void starterTestPasswordValidator() {
        assertFalse(Main.checkForPassword("\"^(?=.*[a-z])(?=.*[A-Z])(?=.*\\\\d).{\" + minLength + \",}$\"", 6));
    }

    void starterTestExtractEmails() {
        assertTrue(Main.extractEmails("\\b[a-zA-Z0-9._%+-]+@(?:mail\\.)?utoronto\\.ca\\b").isEmpty());
    }

    void starterTestCheckForDoubles() {
        assertFalse(Main.checkForDoubles("\".*\" + c + \".*\" + c + \".*\""));
    }
